...
impl Config {
    ...
    pub fn match_subsequence(&self, seq: &[u8], set_idx: usize, pos: usize, offset: Option<usize>) -> Option<(usize, usize)> {
    ...
    pub fn build_barcode(&self, b1_idx: usize, b2_idx: usize, b3_idx: usize, b4_idx: usize) -> Vec<u8> {
        let mut bc = Vec::with_capacity(self.bc1.len() + self.bc2.len() + self.bc3.len() + self.bc4.len());
        bc.extend_from_slice(self.bc1.get_barcode(b1_idx).expect("Invalid barcode index in bc1"));
        bc.extend_from_slice(self.bc2.get_barcode(b2_idx).expect("Invalid barcode index in bc2"));
        bc.extend_from_slice(self.bc3.get_barcode(b3_idx).expect("Invalid barcode index in bc3"));
        bc.extend_from_slice(self.bc4.get_barcode(b4_idx).expect("Invalid barcode index in bc4"));
        bc
    }
}
